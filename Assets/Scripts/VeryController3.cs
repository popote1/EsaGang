...
public class VeryController3 : MonoBehaviour
{
    ...
    public GameObject HitPlayerParticuls;
    public ParticleSystem DashParticul;
    [Header("ThrowObjcts")] public Grabable Grabable;
    public Rigidbody Hand;
    public float Throwforce;
    [Header("Hp")] public int MaxHP = 10;
    ...
    [Header("Graphics")] public SkinnedMeshRenderer TchirtMeshRenderer;
    ...
    private void FixedUpdate()
    {
        if (_outOfControlTimer > 0)
        {
            ...
            if (!_hasHitDash)
            {
                ...
                {
                    if (collider.CompareTag("Player") && collider.GetComponentInParent<VeryController3>() &&
                        collider.GetComponentInParent<VeryController3>().transform != transform)
                    {
                        VeryController3 controller =collider.gameObject.GetComponentInParent<VeryController3>();
                        controller.TakeDamage(DashDamage);
                        Instantiate(HitPlayerParticuls, (transform.position + collider.transform.position) / 2,
                            quaternion.identity);
                        _hasHitDash = true;
                        return;
                    }
    ...
    public void OnGrabObject()
    {
        ...
        else
        {
            ...
            Grabable.Rigidbody.mass = Grabable.Rigidbody.mass * 10;
            Grabable.Rigidbody.AddForce(transform.forward.normalized * (Throwforce * Grabable.Rigidbody.mass),
                ForceMode.Impulse);
